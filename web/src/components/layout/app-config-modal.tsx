...
import { App, Button, Form, Input, Modal, Progress, Radio, Segmented, Select, Tabs } from "antd";
...
export function AppConfigModal() {
    ...
    const modelChannel = config.channels.find((channel) => config.model.startsWith(channel.id)) || config.channels[0];
                ...
                items={[
                    {
                        ...
                        children: (
                            <Form layout="vertical" requiredMark={false}>
								{allowCustomChannel ? (
									<div className="mb-4 flex items-center gap-3 rounded-lg border border-stone-200 p-3 dark:border-stone-800">
										...
										>
											<Radio.Button value="local">本地直连</Radio.Button>
											<Radio.Button value="remote">云端渠道</Radio.Button>
										</Radio.Group>
										<span className="text-xs text-stone-500">{effectiveMode === "local" ? "浏览器直接连接 AI 服务商" : "通过后端代理请求"}</span>
									</div>
								) : null}
