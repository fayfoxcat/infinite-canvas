...
// Async image task types — submit returns taskId, frontend polls /images/tasks/:id
type ImageTaskCreateResponse = { taskId?: string; code?: number; data?: { taskId?: string } | null; msg?: string };
type ImageTaskStatePayload = { status: "pending" } | { status: "completed"; payload: ImageApiResponse } | { status: "failed"; error?: string };
...
export async function requestGeneration(config: AiConfig, prompt: string, options?: RequestOptions) {
	...
	// Remote mode: use async task + polling to bypass Cloudflare 100s timeout
	if (config.channelMode === "remote") {
		const payload = await runImageTask(requestConfig, () =>
			axios.post<ImageTaskCreateResponse>(`${API_BASE}/api/v1/images/generations/async`, { model: requestConfig.model, prompt: withSystemPrompt(requestConfig, prompt), n, ...(quality ? { quality } : {}), ...(requestSize ? { size: requestSize } : {}), response_format: "b64_json", output_format: IMAGE_OUTPUT_FORMAT }, { headers: aiHeaders(requestConfig, "application/json"), signal: options?.signal }),
		);
...
export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], mask?: ReferenceImage, options?: RequestOptions) {
	...
	// Remote mode: use async task + polling
	if (config.channelMode === "remote") {
		const payload = await runImageTask(requestConfig, () =>
			axios.post<ImageTaskCreateResponse>(`${API_BASE}/api/v1/images/edits/async`, formData, { headers: aiHeaders(requestConfig), signal: options?.signal }),
		);
		const images = parseImagePayload(payload);
		refreshRemoteUser(requestConfig);
		return images;
...
// ---- Async image task helpers ----

async function runImageTask(requestConfig: ModelChannel, submit: () => Promise<{ data: ImageTaskCreateResponse }>): Promise<ImageApiResponse> {
	let created: ImageTaskCreateResponse;
	try {
		created = (await submit()).data;
	} catch (error) {
		throw new Error(readAxiosError(error, "请求失败"));
	}
	const taskId = created.taskId || created.data?.taskId;
	if (!taskId) {
		if (typeof created.code === "number" && created.code !== 0) throw new Error(created.msg || "请求失败");
		throw new Error("图片任务创建失败");
	}
	for (let attempt = 0; attempt < IMAGE_TASK_MAX_POLLS; attempt += 1) {
		await delay(IMAGE_TASK_POLL_INTERVAL_MS);
		let state: ImageTaskStatePayload;
		try {
			state = unwrapImageTaskState((await axios.get<ImageTaskStateResponse>(`${API_BASE}/api/v1/images/tasks/${encodeURIComponent(taskId)}`, { headers: aiHeaders(requestConfig) })).data);
		} catch (error) {
			throw new Error(readAxiosError(error, "图片任务查询失败"));
		}
		if (state.status === "completed") return state.payload;
		if (state.status === "failed") throw new Error(state.error || "图片生成失败");
	}
	throw new Error("图片生成超时，请稍后重试");
}

function unwrapImageTaskState(payload: ImageTaskStateResponse): ImageTaskStatePayload {
	if (payload && typeof payload === "object" && "code" in payload && typeof payload.code === "number") {
		if (payload.code !== 0) throw new Error(payload.msg || "图片任务查询失败");
		if (!payload.data) throw new Error("图片任务查询失败");
		return payload.data;
	}
	return payload as ImageTaskStatePayload;
}
